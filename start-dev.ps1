...
try {
    ...
    Write-Host "  [WARNING] PostgreSQL command not found (may still work if service is running)" -ForegroundColor Yellow
}
...
if (Test-Path "backend\.env") {
    ...
    Write-Host "  [OK] Created backend/.env with default values" -ForegroundColor Green
    Write-Host "  [WARNING] Edit backend/.env with your actual PostgreSQL credentials!" -ForegroundColor Yellow
}
...
if ($port5001) {
    Write-Host "  [WARNING] Port 5001 (Backend) is already in use" -ForegroundColor Yellow
    ...
    Write-Host "  [OK] Port 5001 available" -ForegroundColor Green
}
