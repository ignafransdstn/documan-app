...
async function createMasterDocument(data, filePath) {
  try {
    ...
    console.log(`✅ Master: ${data.title}`);
    return response.data;
  } catch (error) {
    console.error(`❌ Gagal: ${data.title}`, error.response?.data?.message || error.message);
    return null;
  }
}

async function createSubDocument(parentId, data, filePath, index) {
  try {
    const form = new FormData();
    form.append('parentDocumentId', parentId);
    form.append('subDocumentNo', `SUB-${String(index + 1).padStart(3, '0')}`);
    form.append('title', data.title);
    form.append('location', data.location);
    form.append('description', data.description);
    form.append('document', fs.createReadStream(filePath));
...
async function generateDocuments() {
  try {
    ...
    for (let i = 0; i < masterDocuments.length && totalCreated < 25; i++) {
      ...
      // Create dummy text file untuk master
      const masterFilePath = path.join(uploadsDir, `temp-master-${i}.txt`);
      createDummyFile(masterFilePath, `Dokumen: ${masterData.title}\n\nLokasi: ${masterData.location}\n\nDeskripsi: ${masterData.description}\n\nTanggal: ${new Date().toISOString()}`);
      
      console.log(`\n📄 [${totalCreated + 1}] ${masterData.title}`);
      const masterDoc = await createMasterDocument(masterData, masterFilePath);
      ...
      // Randomly create 0-2 sub-documents
      ...
      for (let j = 0; j < subCount && totalCreated < 25; j++) {
        ...
        const subFilePath = path.join(uploadsDir, `temp-sub-${i}-${j}.txt`);
        createDummyFile(subFilePath, `Sub-Dokumen: ${subData.title}\n\nLokasi: ${subData.location}\n\nDeskripsi: ${subData.description}\n\nTanggal: ${new Date().toISOString()}`);
        
        const subDoc = await createSubDocument(masterDoc.id, subData, subFilePath, j);
        
        fs.unlinkSync(subFilePath);
        
        if (subDoc) totalCreated++;
    ...
    console.log('📊 Silakan refresh halaman Documents untuk melihat hasilnya.\n');
    process.exit(0);
  } catch (error) {
    console.error('\n❌ Error:', error.message);
    process.exit(1);
